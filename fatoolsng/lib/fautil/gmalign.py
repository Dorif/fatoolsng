...
class ZFunc(object):
    ...
    def get_pairs(self, z):
        f = poly1d(z)
        pairings = pair_f(f, self.rtimes, self.sizes, self.similarity,
                          deviation=True)
    ...
    def __call__(self, z):
        ...
        # prepare z function
        f = poly1d(z)
        pairs = pair_f(f, self.rtimes, self.sizes, self.similarity,
                       deviation=True)
...
def align_sh(peaks, ladder):
    # stochastic-heuristic method this is a combination of differential
    # evolution method on a subset of data and then minimization for
    # further processing
