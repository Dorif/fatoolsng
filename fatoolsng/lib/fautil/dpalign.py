...
def annotate(M, ladders, peaks):
    M_ = insert(M, 0, values=array([x.rtime for x in peaks]), axis=0)
    return insert(M_, 0, values=array([0] + ladders), axis=1)
...
def align_peaks(ladders, peaks, z, rss):
    ...
    while True:
        ...
        S = generate_scores(ladders, peaks, poly1d(z))
    ...
    return dpscore, rss, z, sized_peaks, annotate(S, ladders,
                                                  peaks), result['D']


def create_scoring_function(A, B):
    def _scoring(ladder, peak, height):
        return ((ladder * A + B) - peak)/height
    return _scoring
...
def generate_scores(ladders, peaks, func, tolerance=4):
    ...
    for ladder in ladders:
        for peak in peaks:
            ...
            similarity = (log10(peak.height/highest_peak) + ladder_N)/ladder_N
            M[row][col] = similarity * exp(-((size-ladder)/(tolerance))**2 / 2)
            # print(ladder, peak)
...
def plot_z(peaks, ladders, z):
    ...
    # x = linspace(0, peaks[-1].rtime + 100)
    x = linspace(0, max(ladders))
    p = poly1d(z)
    y_p = p(x)
...
def dp(S, gap_penalty, peak_penalty=0):
    ...
    for i in range(1, row_length+1):
        for j in range(1, col_length+1):
            ...
            if j == col_length:
                ...
                penalty = gap_penalty
            darray = [D[i-1, j-1]+S[i-1, j-1], D[i-1, j]+penalty,
                      D[i, j-1] + peak_penalty]
            D[i, j] = max(darray)
    ...
    return {'p': p, 'q': q, 'trace': trace, 'matches': matches, 'D': D,
            'phi': trace_matrix}
