from fatoolsng.lib import dpalign as dp
...
def fast_align(trace, initial_peaks, avg_height, ladders, all_peaks):
    ...
    dp_score, dp_rss, dp_z, dp_peaks, S, D = align(initial_peaks, ladders,
                                                   peak_pairs)
    ...
    for i in range(0, I):
        for j in range(1, J):
            ...
            while True:
                ...
                dp_score, dp_rss, dp_z, dp_peaks, S, D = align(peaks, ladders,
                                                               peak_pairs)
                print(' * dp_score =', dp_score)
...
def greedy_1b(trace, peaks, avg_height, ladders, all_peaks, min_rss):
    ...
    if dp_rss > 70:
        ...
        dp_score2, dp_rss2, dp_z2, dp_peaks2, S2, D2 = align(peaks, ladders,
                                                             peak_pairs)
...
def iter_align(peaks, ladders, peak_pairs, all_peaks):
    ...
    while True:
        ...
        dp_score, dp_rss, dp_z, dp_peaks, S, D = simple_align(peaks, ladders,
                                                              peak_pairs)
        # print(' * dp_score =', dp_score )
...
def greedy_1a(trace, peaks, avg_height, ladders, all_peaks, min_rss):
    ...
    for i in range(0, I):
        for j in range(1, J):
            ...
            dp_score, dp_rss, dp_z, dp_peaks, S, D = dp.align_peaks(ladders,
                                                                    peaks, z,
                                                                    rss)
            print('==>', dp_score, dp_rss, len(dp_peaks))
            # pprint.pprint(dp_peaks)
...
def shift_peak_pairs(peak_pairs, z):
    ...
    for (idx, peak_pair) in enumerate(peak_pairs):
        deviation = abs(f(peak_pair[1].rtime) - peak_pair[0])
        cerr('  %2d  %3d  %6d  %5.2f' % (idx, peak_pair[0], peak_pair[1].rtime,
                                         deviation))
        if deviation > max_value:
            max_value = deviation
            max_i = idx
    ...
    for (idx, peak_pair) in enumerate(new_peak_pairs):
        ...
        cerr('  %2d  %3d  %6d  %5.2f' % (idx, peak_pair[1], peak_pair[0],
                                         deviation))
...
def generate_peak_pairs(ladders, peaks, start_pos, end_pos):
    ...
    print('min_size = %d; start = %d; end = %d;' % (min_size, start_pos,
                                                    end_pos))
    peak_pairs = []
