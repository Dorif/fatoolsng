...
from jax.numpy import (array, sum, mean, percentile, polyfit, isnan, zeros,
                       median, linspace)
...
def filter_retention_time(ladders, peaks, parameter):
    ...
    print("peak count: %d separation_slope: %2.3f" % (len(peaks),
                                                      separation_slope))
    ...
    while idx < len(peaks):
        ...
        if prev_peak.rtime - peaks[idx].rtime > min_time_separation:
...
def peak_fit_and_align(simple_fits, ladders):
    ...
    dp_fits = []
    for (initial_z, initial_rss, initial_peaks, height,
         paired_peaks) in simple_fits[:6]:

        print('=> initial RSS: %3.3f' % initial_rss)
        dp_score, dp_rss, dp_z, dp_peaks, S, D = align_peaks(ladders,
                                                             initial_peaks,
                                                             initial_z=initial_z)
        dp_fits.append((dp_score, dp_rss, dp_z, dp_peaks, initial_peaks,
                        height, S, D, paired_peaks))
...
def xxx_scan_ladder_peaks(channel, ladders, parameter=None):
    ...
    # sanity check
    if (len(peak_sets) == 0 or
        len(peak_sets[0][1]) < len(ladders)*parameter.min_peak_number):
        return (None, -1, 0,
                ('Not enough initial signals',
                 'Too much noises or too few signals'), [])
    ...
    for (height, peaks) in peak_sets:
        ...
        if (not proper_peaks or
            (len(proper_peaks) < len(ladders) * parameter.min_peak_number or
             len(proper_peaks) > len(ladders) * 1.5)):
            continue
        separated_peaksets.append((height, proper_peaks))

    if not separated_peaksets:
        return (None, -1, 0, ('Not enough filtered peaks',
                              'Bad peak separation time'), [])
    ...
    while simple_fits[i:i+3] and (optimal_rss > len(ladders)*1.5 and
                                  optimal_dpscore < len(ladders)):
    ...
    score, reports = score_ladder(optimal_rss, len(optimal_peaks),
                                  len(ladders))
    ...
    print(' => aligned peaks: %d of %d ladders' % (len(optimal_peaks),
                                                   len(ladders)))
...
def scan_ladder_peaks(channel, ladders, parameter=None):
    ...
    # sanity check
    if len(peak_sets) == 0:
        return (None, -1, 0, -1, ('Not enough initial signals',
                                  'Too much noises or too few signals'), [])
    ...
    if not separated_peaks:
        return (None, -1, 0, -1, ('Not enough filtered peaks',
                                  'Bad peak separation time'), [])
    ...
    for (peak_len, proper_peaks, min_height) in separated_peaks:
        ...
        dpscore, rss, z, aligned_peaks = adaptive_peak_alignment(ladders,
                                                                 proper_peaks)
        # rss, z, aligned_peaks = align_peaks(ladders, proper_peaks)
        best_fits.append((dpscore, rss, z, aligned_peaks, min_height,
                          proper_peaks))
    ...
    print('=> aligned peaks: %d of %d ladders' % (len(aligned_peaks),
                                                  len(ladders)))
...
def scan_ladder_peaks_xxx(channel, ladders, parameter=None):
    ...
    for min_height in parameter.ladder_height_win:
        ...
        if len(peaks) == prev_peak_len or len(peaks) > len(ladders)*2:
            # too much noises
            continue
    ...
    if not filtered_peaks:
        # no available data
        return (None, -1, 0, ('Not enough initial signals',
                              'Too much noises or too few signals'), [])
    ...
    for (min_height, peaks) in filtered_peaks:
        proper_peaks = filter_retention_time(ladders, peaks, parameter)
        if (not proper_peaks or
            len(proper_peaks) < len(ladders)*parameter.min_peak_number):
            continue
    ...
    print('=> aligned peaks: %d of %d ladders' % (len(optimal_peaks),
                                                  len(ladders)))
    pprint(optimal_peaks)
    return (optimal_z, optimal_rss, score, reports,
            [p for (s, p) in optimal_peaks])
...
def filter_separation_time(ladders, peaks, parameter):
    ...
    while idx < len(peaks):
        prev_peak = ladder_alleles.pop()
        # print('comparing peaks: %d & %d' % (prev_peak.peak, peaks[idx].peak))
        if prev_peak.peak - peaks[idx].peak > min_time_separation:
            # minimum time separation
            ladder_alleles.append(prev_peak)
            ladder_alleles.append(peaks[idx])
        else:
...
def adaptive_peak_alignment(ladders, peaks):
    ...
    for (rss, z, aligned_peaks) in adaptive_results[:3]:
        ...
        dp_score, dp_rss, dp_z, dp_peaks, S, D = align_peaks(ladders, peaks,
                                                             aligned_peaks)
        if dp_rss < rss:
            print(' => DP alignment replaced RSS: %3.2f -> %3.2f' % (rss,
                                                                     dp_rss))
...
def scan_ladder_peaks_xxx(channel, ladders, parameter=None):
    ...
    for min_height in parameter.ladder_height_win:
        ...
        if last_peak_no == len(peaks):
            # if current peak number is similar to above, just continue
            # increase min_height
            continue
    ...
    # assumption:
    # - no drop peaks
    # - the maximum number of missing peaks at the later
    # retention time is 5 (shifted out)
    ...
    return (optimal_z, optimal_rss, quality_score, reports,
            [p for (s, p) in optimal_peaks])
...
def scan_peaks(channel, parameter=None):
    ...
    for peak in peaks:
        ...
        allele = channel.get_allele_class()(rtime=int(peak[0]),
                                            height=int(peak[1]), area=peak[2],
                                            brtime=peak[3], ertime=peak[4],
...
def call_peaks(alleles, method, ladders, parameter=None):
    ...
    for allele in alleles:
        ...
        # only call peaks within the ladders, otherwise
        # just assigned -1 to the value
        ...
        # remove small fragment size
        if (allele.size < parameter.min_size or
            allele.size > parameter.max_size):
            allele.type = 'peak-unassigned'
            continue
        allele.type = 'peak-called'
        called_alleles.append(allele)

    # classify alleles to peak-stutter or peak-called for the list of alleles
    # that were not peak-unassigned
...
def bin_peaks(channel, marker):
    """ bin peaks that have been called (have peak size > 0),
        return the list of alleles, after allele has been set as peak-bin
        or peak-artifact
    """
            ...
            for k in range(len(channel_peaks)):
                ...
                for peak_r in channel_peaks[k]:
                    ...
                    if is_overlap(peak, peak_r):
                        # find whether the height of any peak is inside
                        # any peak
...
def find_peaks(signal, params, ssignal=None):
    ...
    indices = find_peaks_cwt(signal, params.cwt_widths,
                             min_snr=params.cwt_min_snr)
    ...
    for idx in indices:
        for i in range(3, -1, -1):
            try:
                height, index = max([(signal[i], i) for i in range(idx-3,
                                                                   idx+3)])
            except IndexError:
                continue
            break
    ...
    threshold = percentile(ssignal, 75)
    if threshold < 0:
        threshold = -1
    peaks = []
    # pprint(raw_peaks)
...
def half_area(y, threshold):
    ...
    shared = False
    area = y[0]
    edge = float(sum(y[0:winsize]))/winsize
    old_edge = 2 * edge
    ...
    while edge > area * threshold and edge < old_edge and index < limit:
        old_edge = edge
        area += y[index]
        edge = float(sum(y[index:index+winsize]))/winsize
        index += 1
    if edge >= old_edge:
        shared = True
    index -= 1
...
def estimate_slope(ladders, peaks):
    ...
    sampled_range_ladders = range_ladders[int(N_ladders/3): int(2*N_peaks/3)]
    sampled_range_peaks = range_peaks[int(N_peaks/3): int(2*N_peaks/3)]

    slope = mean(sampled_range_peaks) / mean(sampled_range_ladders)
    return 1.0/slope
...
def align_ladder_peaks(ladders, peaks):
    ...
    sampled_range_ladders = range_ladders[int(N_ladders/4): int(N_peaks*0.75)]
    sampled_range_peaks = range_peaks[int(N_peaks/4): int(N_peaks * 0.75)]
    ...
    for i in range(-3, 3):
        ...
        z, rss, peak_assignment = fit_ladder_peaks(ladders, peaks, initial_y,
                                                   i)
...
def fit_ladder_peaks(ladders, peaks, initial_y, index):
    ...
    param, covar, rss = linear_fit_ladder_peaks(ladders, peaks, initial_y,
                                                index)

    # reassign ladders with peaks for consecutive peakfit
    peak_assignment = assign_ladder_peak(ladders, peaks,
                                         lambda x: initial_y(x, param))
...
def linear_fit_ladder_peaks(ladders, peaks, initial_y, idx):
    ...
    p0 = 1
    x = array(ladders[:data_length])
    y = array([p.peak for p in peaks][:data_length])
    ...
    # plot the fitt
    print('idx = %d, param = %8.3f, covar = %8.3f, rss = %8.3f' % (idx, param,
                                                                   covar, rss))
    x_lines = linspace(x[0], x[-1], 1000)
...
def linear_fit(ladders, peaks):
    ...
    for i in range(-10, 10):
        ...
        linear_fits.append((array([A, param[0]]), rss))
...
def align_and_fit(ladders, peaks, func, idx):
    ...
    data_length = min(len(ladders), len(peaks))
    p0 = 1
    y = array(ladders[:data_length])
    x = array([p.rtime for p in peaks][:data_length])
    # linsq = leastsq(initial_residuals, p0, args = (x, y))
...
def find_ladder_peak(channel, N, parameter=None):
    ...
    if parameter.height > 0:
        ...
        return filter_peak_number(initial_peaks,
                                  N + parameter.additional_peaks, N)
...
def scan_ladder_peak_2(channel, ladder, parameter=None, find_peaks=True):
    ...
    if find_peaks:
        ...
        # use the old peaks, set all into peak-unassigned, and
        # redo the peak alignment
        pass
    ...
    # for each results, find the least RSS and the highest DP score and
    # quality score
