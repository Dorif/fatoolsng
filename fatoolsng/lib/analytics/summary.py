...
def summarize_allele_df(allele_df):
    ...
    for (marker_id, allele), df in allele_df.grouped_df:
        ...
        allele_list[marker_id].append((allele, len(df), mean(df['height']),
                                       min(df['size']), max(df['size']),
                                       list(df['sample_id']), mean(df['size']),
                                       (df['size'], df['height'])))
    ...
    for marker_id in allele_list:
        ...
        allele_params = [(allele, count/total_allele, count, mean_height,
                          min_size, max_size, max_size-min_size, sample_ids,
                          mean_size, items) for (allele, count, mean_height,
                                                 min_size, max_size,
                                                 sample_ids, mean_size, items)
                         in alleles]

        delta_status = check_delta(allele_params)

        results[marker_id] = dict(code=marker_id,
...
def summarize_bins(analytical_sets):
    ...
    for marker_id, marker_summary in marker_summaries.items():
        ...
        for (allele_bin, sizes) in marker_summary.items():
            # not found
            percentiles = percentile(sizes, [25, 75])
            empirical_bins[allele_bin] = [int(allele_bin),
                                          round(float(mean(sizes)), 3),
