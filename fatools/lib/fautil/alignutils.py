...
def generate_similarity(peaks):
    ...
    N = len(rfus)
    similarity = list([(log10(rfu/highest_rfu) + N)/N if rfu < highest_rfu
                       else 1.0 for rfu in rfus])
...
def generate_scores(sizes, rtimes, similarity, func, tolerance=4):
    ...
    for c in range(0, len(rtimes)):
        ...
        for r in range(0, len(sizes)):
            M[r][c] = similarity[c]*exp(-((size-sizes[r])/(tolerance))**2 / 2)
...
def plot(rtimes, sizes, z, peak_pairs):
    ...
    range_x = max_x - min_x
    x = linspace(min_x, max_x, range_x+1)
    f = poly1d(z)
    y = f(x)
...
def align_dp(rtimes, sizes, similarity, z, rss, order=3):
    ...
    while True:
        ...
        S = generate_scores(sizes, rtimes, similarity, poly1d(z))
